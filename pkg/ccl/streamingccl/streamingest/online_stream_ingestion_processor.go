...
var onlineStreamIngestionResultType = []*types.T{
	types.Bytes, // jobspb.ResolvedSpans
}

// onlineStreamIngestionProcessor started life as a copy/pasta fork of the
...
type onlineStreamIngestionProcessor struct {
...
var (
	_ execinfra.Processor = &onlineStreamIngestionProcessor{}
	_ execinfra.RowSource = &onlineStreamIngestionProcessor{}
)

const onlineStreamIngestionProcessorName = "online-stream-ingestion-processor"

func newOnlineStreamIngestionDataProcessor(
	ctx context.Context,
	flowCtx *execinfra.FlowCtx,
	processorID int32,
	spec execinfrapb.ReplicationSpec,
	...
	rfCache := cdcevent.NewFixedRowFetcherCache(ctx, flowCtx.Codec(), flowCtx.Cfg.Settings, descs)
	decoder := cdcevent.NewEventDecoderWithCache(ctx, rfCache, false, false)

	sip := &onlineStreamIngestionProcessor{
		flowCtx: flowCtx,
		spec:    spec,
	...
	if err := sip.Init(ctx, sip, post, onlineStreamIngestionResultType, flowCtx, processorID, nil, /* memMonitor */
...
func (sip *onlineStreamIngestionProcessor) Start(ctx context.Context) {
	ctx = logtags.AddTag(ctx, "job", sip.spec.JobID)

	ctx = sip.StartInternal(ctx, onlineStreamIngestionProcessorName)
	...
	log.Infof(ctx, "starting online stream ingestion processors (partitions: %d)", len(sip.spec.PartitionSpecs))
	...
	for _, partitionSpec := range sip.spec.PartitionSpecs {
		...
		sub, err := streamClient.Subscribe(ctx, streampb.StreamID(sip.spec.StreamID), int32(sip.flowCtx.NodeID.SQLInstanceID()), token,
			sip.spec.InitialScanTimestamp, sip.frontier, true /* withFiltering */)
...
func (sip *onlineStreamIngestionProcessor) Next() (
	rowenc.EncDatumRow,
...
func (sip *onlineStreamIngestionProcessor) MoveToDrainingAndLogError(err error) {
	if err != nil {
...
func (sip *onlineStreamIngestionProcessor) MustBeStreaming() bool {
	return true
}

// ConsumerClosed is part of the RowSource interface.
func (sip *onlineStreamIngestionProcessor) ConsumerClosed() {
	sip.close()
}

func (sip *onlineStreamIngestionProcessor) close() {
	if sip.Closed {
		return
	}
...
func (sip *onlineStreamIngestionProcessor) sendError(err error) {
	if err == nil {
		return
	}
	select {
...
func (sip *onlineStreamIngestionProcessor) flushLoop(_ context.Context) error {
	for {
...
func (sip *onlineStreamIngestionProcessor) onFlushUpdateMetricUpdate(
...
func (sip *onlineStreamIngestionProcessor) bufferKVs(kvs []roachpb.KeyValue) error {
	if kvs == nil {
...
func (sip *onlineStreamIngestionProcessor) bufferCheckpoint(event partitionEvent) error {
	if streamingKnobs, ok := sip.FlowCtx.TestingKnobs().StreamingTestingKnobs.(*sql.StreamingTestingKnobs); ok {
...
func (sip *onlineStreamIngestionProcessor) flushBuffer(
	b flushableBuffer,
...
//
// These implement partial last-write-wins semantics. We assume that the table has an crdb_internal_origin_timestamp column
// defined as:
//
//	crdb_internal_origin_timestamp DECIMAL NOT VISIBLE DEFAULT NULL ON UPDATE NULL
//
// This row is explicitly set by the INSERT query using the MVCC timestamp of the inbound write.
//
// Known issues:
//
// 1. An UPDATE and a DELETE may be applied out of order because we have no way from SQL of knowing the write timestamp of the deletion tombstone.
...
func init() {
	rowexec.NewOnlineStreamIngestionProcessor = newOnlineStreamIngestionDataProcessor
}
